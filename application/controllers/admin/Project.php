...
class Project extends Admin_Controller {
    ...
    function index()
    {
        ...
        {
            $filters['order_id'] = $this->input->get('order_id', TRUE);
        }
...
/*        if ($this->input->get('first_name'))
        ...
        if ($this->input->post())
        {
            ...
            else
            {
                ...
                {
                    $filter .= "&order_id=" . $this->input->post('username', TRUE);
                }
    ...
    function edit($id = NULL)
    {
        ...
        /*
        $this->form_validation->set_error_delimiters($this->config->item('error_delimeter_left'), $this->config->item('error_delimeter_right'));
        $this->form_validation->set_rules('username', lang('users input username'), 'required|trim|min_length[5]|max_length[30]|callback__check_username[' . $projects['username'] . ']');
        $this->form_validation->set_rules('first_name', lang('users input first_name'), 'required|trim|min_length[2]|max_length[32]');
        $this->form_validation->set_rules('last_name', lang('users input last_name'), 'required|trim|min_length[2]|max_length[32]');
        $this->form_validation->set_rules('email', lang('users input email'), 'required|trim|max_length[128]|valid_email|callback__check_email[' . $projects['email'] . ']');
        $this->form_validation->set_rules('language', lang('users input language'), 'required|trim');
        $this->form_validation->set_rules('status', lang('users input status'), 'required|numeric');
        $this->form_validation->set_rules('is_admin', lang('users input is_admin'), 'required|numeric');
    ...
    function delete($id = NULL)
    {
        ...
        {
            // get user details
            $projects = $this->projects_model->get_user($id);

            if ($projects)
            {
                // soft-delete the user
                $delete = $this->projects_model->delete_user($id);

                if ($delete)
                {
                    $this->session->set_flashdata('message', sprintf(lang('users msg delete_user'), $projects['first_name'] . " " . $projects['last_name']));
                }
                else
                {
                    $this->session->set_flashdata('error', sprintf(lang('users error delete_user'), $projects['first_name'] . " " . $projects['last_name']));
                }
            }
            else
            {
    ...
    function export()
    {
        ...
        // get all users
        $projectss = $this->projects_model->get_all(0, 0, $filters, $sort, $dir);

        if ($projectss['total'] > 0)
        {
            // manipulate the output array
            foreach ($projectss['results'] as $key=>$projects)
            {
                unset($projectss['results'][$key]['password']);
                unset($projectss['results'][$key]['deleted']);

                if ($projects['status'] == 0)
                {
                    $projectss['results'][$key]['status'] = lang('admin input inactive');
                }
                else
                {
                    $projectss['results'][$key]['status'] = lang('admin input active');
                }
            }

            // export the file
            array_to_csv($projectss['results'], "users");
        }
        else
        {
            // nothing to export
     ...
     * @return int|boolean
     */
    function _check_username($projectsname, $current)
    {
        if (trim($projectsname) != trim($current) && $this->projects_model->username_exists($projectsname))
        {
            $this->form_validation->set_message('_check_username', sprintf(lang('users error username_exists'), $projectsname));
            return FALSE;
        }
        else
        {
    ...
    {
        if (trim($email) != trim($current) && $this->projects_model->email_exists($email))
        {
